    ...
    try:
        ...
        if not approved_patch_plan:
            return {
                'statusCode': 200,
                'scan_id': scan_id,
                'message': 'No approved patches found — skipping deployment.',
                'fixes_applied': 0
            }
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            commit_message = f"""🔒 Automated security fixes (Scan ID: {scan_id})
...
"""
        ...
        pr_title = f"🤖 Automated Security Fixes - Scan {scan_id}"
...
def generate_pr_description(scan_data, scan_id):
    patch_plan = scan_data.get("patch_plan", [])
    description = f"""## 🛡️ Automated Security Patch Report (Scan ID: {scan_id})
...
✅ **All fixes have been automatically applied and tested for syntax correctness.**  
⚠️ Please review before merging to ensure business logic remains intact.
